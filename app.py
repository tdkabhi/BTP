...
app.config['UPLOAD_FOLDER'] = 'uploads/'  # Directory where uploaded zip files are stored
app.config['MAX_CONTENT_LENGTH'] = 600 * 1024 * 1024  # 600 MB max upload
...
def upload_file():
    ...
    with ZipFile(file_path, 'r') as zip_ref:
        zip_ref.extractall('train1')  # Extracting to the image folder
...
active_models = set(models.keys())
image_folder = 'train1'
cumulative_emissions = {name: 0 for name in models_config.keys()}
...
def emit_model_emissions():
    with ThreadPoolExecutor(max_workers=len(models)) as executor:
        while True:
            ...
            for image_path in images:
                futures = {model_name: executor.submit(process_image, model_name, models[model_name], image_path) for model_name in active_models}
                results = [future.result() for future in futures.values() if future.result() is not None]
                if results:
                    socketio.emit('model_emissions', {'image': os.path.basename(image_path), 'results': results})
